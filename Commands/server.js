    ...
    if(process.env.owner ===message.author.id || db.get(`ownermd.${message.author.id}`) === true) {
        ...
        .setDescription( client.guilds.cache
          ...
          .map((r, i) => `${i+1} - ${r.name} [${r.memberCount}]`)
            ...
            .setFooter(`${page}/${Math.ceil( client.guilds.cache.size.toLocaleString()/ 15) || 1}・IrysBot `)
...
tt .setDescription( client.guilds.cache
    .sort((a, b) => b.memberCount - a.memberCount)
                  ...
                  .map((r, i) => `${i+1} - ${r.name} [${r.memberCount}]`)
                    ...
                    tt.setFooter(`${page}/${Math.ceil( client.guilds.cache.size.toLocaleString()/ 15)}・IrysBot `)
                    tt.setColor(color)
